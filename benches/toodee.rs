    ...
    for dims in [(32usize, 20usize), (320, 200), (640, 480)].iter() {
        ...
        group.bench_with_input(BenchmarkId::new("rows_mut", size), &size, |b, _| {
            b.iter(|| {
                black_box(
                    view.rows_mut()
                        .map(|r| r.iter_mut().map(|x| *x).sum::<u32>())
                        .sum::<u32>(),
                )
            });
        });
    ...
    for &size in [100usize, 200, 300, 400].iter() {
        ...
        // insert_row
        {
            ...
            group.bench_with_input(BenchmarkId::new("insert_row", size), &size, |b, _| {
                b.iter_batched(
                    || {
                        let mut tmp = toodee.clone();
                        tmp.reserve(size);
                        (tmp, new_data.clone())
                    },
                    |(mut data, new_data)| data.insert_row(0, new_data),
        ...
        // insert_col
        {
            ...
            group.bench_with_input(BenchmarkId::new("insert_col", size), &size, |b, _| {
                b.iter_batched(
                    ...
                    |(mut data, new_data)| data.insert_col(0, new_data),
                    BatchSize::LargeInput,
                )
            });
        }
        ...
        // remove_row
        {
            group.bench_with_input(BenchmarkId::new("remove_row", size), &size, |b, _| {
                b.iter_batched(
                    || toodee.clone(),
                    |mut data| {
                        let drain = data.remove_row(0);
                        black_box(drain.sum::<u32>());
                    },
                    BatchSize::LargeInput,
                )
            });
